 ...
 * @since 1.4
 */
public class EnhancedMessagingMessageListenerAdapter extends AbstractAdaptableMessageListener {
	...
	public EnhancedMessagingMessageListenerAdapter() {
		this(null, null);
	}

	public EnhancedMessagingMessageListenerAdapter(Object bean, Method method) {
		this(bean, method, false, null);
	}

	public EnhancedMessagingMessageListenerAdapter(Object bean, Method method, boolean returnExceptions,
										   RabbitListenerErrorHandler errorHandler) {
		this(bean, method, returnExceptions, errorHandler, false);
	}

	protected EnhancedMessagingMessageListenerAdapter(Object bean, Method method, boolean returnExceptions,
	...
	public void onMessage(org.springframework.amqp.core.Message amqpMessage, Channel channel) throws Exception { // NOSONAR
		...
		try {
			...
			// 设置灰度发布参数
			RabbitGrayContextUtil.setGrayContext(amqpMessage.getMessageProperties());

			// 消息灰度版本匹配校验
			boolean isMatchVersion = GrayRabbitLoadBalancerSupport.matchMessageVersion(amqpMessage);
			if (!isMatchVersion) {
				// 消息校验失败，手动NACK，重新入队
			...
			// 清空灰度发布参数
			RabbitGrayContextUtil.clearGrayContext();
		}
	...
	protected final class MessagingMessageConverterAdapter extends MessagingMessageConverter {
		...
		private Type determineInferredType() { // NOSONAR - complexity
			...
			for (int i = 0; i < this.method.getParameterCount(); i++) {
				...
				if (isHeaderOrHeaders && isPayload && EnhancedMessagingMessageListenerAdapter.this.logger.isWarnEnabled()) {
					EnhancedMessagingMessageListenerAdapter.this.logger.warn(this.method.getName()
				...
				if (isEligibleParameter(methodParameter) // NOSONAR
					...
					if (genericParameterType == null) {
						...
						if (EnhancedMessagingMessageListenerAdapter.this.logger.isDebugEnabled()) {
							EnhancedMessagingMessageListenerAdapter.this.logger
